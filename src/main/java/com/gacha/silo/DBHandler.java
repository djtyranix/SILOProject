...
public class DBHandler {
    ...
    public String[] checkItems(String itemStr) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                res = new String[] { id, rs.getString("barcode"), rs.getString("title"), rs.getString("description"),
                        rs.getString("manufacturer"), rs.getString("url"), numberOfStocks };
            }
    ...
    public String simpanSuratJalan(DeliveryNote deliveryNote) {
        ...
        try {
            ...
            PreparedStatement st = con.prepareStatement(
                    "INSERT INTO deliverynote(invoice_id, custName, custEmail, orderDate, deliveryDate, items, status) VALUES (?, ?, ?, ?, ?, ?, ?)",
    ...
    public Item getItem(int idItem) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                item = new Item(id, barcode, rs.getString("title"), rs.getString("description"),
                        rs.getString("manufacturer"), rs.getString("url"), stocks);
            }
    ...
    public String simpanItem(Item item) {
        ...
        try {
            Connection con = initDB();
            PreparedStatement st = con.prepareStatement(
                    "INSERT INTO item(barcode, title, description, manufacturer, url, numberOfStocks) VALUES (?, ?, ?, ?, ?, ?)",
                    Statement.RETURN_GENERATED_KEYS);
    ...
    public String simpanInvoice(Invoice invoice) {
        ...
        try {
            Connection con = initDB();
            PreparedStatement st = con.prepareStatement(
                    "INSERT INTO invoice(poNumber, supplierName, orderDate, deliveryDate, status) VALUES (?, ?, ?, ?, ?)",
                    Statement.RETURN_GENERATED_KEYS);
    ...
    public ArrayList<DeliveryNote> getAllDeliveryNotes() {
        ...
        try {
            ...
            while (rs.next()) {
                String id = Integer.toString(rs.getInt("id"));
                curDeliveryNote = new DeliveryNote(id, rs.getString(2), rs.getString(3), rs.getString(4),
    ...
    public ArrayList<Item> getAllItems() {
        ...
        try {
            ...
            while (rs.next()) {
                String id = Integer.toString(rs.getInt("id"));
                curItem = new Item(id, rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5),
    ...
    public ArrayList<Invoice> getAllInvoices() {
        ...
        try {
            Connection con = initDB();
            PreparedStatement st = con.prepareStatement("SELECT * FROM invoice");
            ResultSet rs = st.executeQuery();

            while (rs.next()) {
                String id = Integer.toString(rs.getInt("id"));
                curInvoice = new Invoice(id, rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5),
                        rs.getInt(6));

                invoices.add(curInvoice);
            }
        } catch (SQLException | ClassNotFoundException e) {
            e.printStackTrace();
        }
    ...
    public ArrayList<Invoice> searchInvoice(String keyword) {
        ...
        try {
            Connection con = initDB();
            PreparedStatement st = con.prepareStatement("SELECT * FROM invoice WHERE poNumber = ? OR id = ?");
